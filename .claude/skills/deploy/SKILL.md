...
**Server-side scanner mapping (since 0.8.36, #69):** on pz-server set `$env:DEVBRIDGE_SERIAL_BRIDGES = "pjkeb-client=COM20,pjsln-client=COM22"` (comma list of `client_id=COMn[:baud]`, baud default 9600) before `irm|iex` with `DEVBRIDGE_MODE=server`. A malformed value aborts the installer BEFORE anything is stopped — in ANY mode (it is parsed before the mode is resolved); a valid value is ignored with a warning in client mode. Fresh config → `[[server.serial_bridges]]` blocks after `[jobs]`; preserved config → only client_ids (case-sensitive) that have NO mapping yet are appended after the last existing mapping (existing mappings are never changed; a port already mapped to another client is skipped with a warning naming that client; an inline `serial_bridges = [...]` config is left untouched with a warning). If the virtual port is missing from `HKLM:\HARDWARE\DEVICEMAP\SERIALCOMM` the installer only WARNS with the exact `setupc.exe --silent install PortName=COMn,EmuBR=yes PortName=COMn+1,EmuBR=yes` command — com0com pairs are never created automatically (see `.claude/skills/serial-bridge/SKILL.md`).

**Client restart = orphaned jobs closed (since 0.8.36, #77):** a client that starts finds any job a previous process left `downloading`/`printing` and marks it `failed` (`error_detail = "interrupted: client service restarted"`, a `failed` job event, WARN log `interrupted in-flight jobs marked failed job_ids=[...]`). So `/api/status` `active_jobs` > 0 for longer than `print_timeout_secs` (default 1800 s) with nothing printing means the client has not been restarted since the process that owned those jobs died — restart the `DevBridgeService` task, never edit `devbridge.db` by hand.
