...
**Server-side scanner mapping (since 0.8.36, #69):** on pz-server set `$env:DEVBRIDGE_SERIAL_BRIDGES = "pjkeb-client=COM20,pjsln-client=COM22"` (comma list of `client_id=COMn[:baud]`, baud default 9600) before `irm|iex` with `DEVBRIDGE_MODE=server`. A malformed value aborts the installer BEFORE anything is stopped. Fresh config → `[[server.serial_bridges]]` blocks after `[jobs]`; preserved config → only client_ids that have NO mapping yet are appended (existing mappings are never changed, a port already mapped to another client is skipped with a warning). Ignored (with a warning) in client mode. If the virtual port is missing from `HKLM:\HARDWARE\DEVICEMAP\SERIALCOMM` the installer only WARNS with the exact `setupc.exe --silent install PortName=COMn,EmuBR=yes PortName=COMn+1,EmuBR=yes` command — com0com pairs are never created automatically (see `.claude/skills/serial-bridge/SKILL.md`).

**Client restart = orphaned jobs closed (since 0.8.36, #77):** a client that starts finds any job a previous process left `downloading`/`printing` and marks it `failed` (`error_detail = "interrupted: client service restarted"`, log `marked interrupted client jobs as failed count=N`). `/api/status` `active_jobs` stuck > 0 with no print running therefore means the service has NOT been restarted since the kill — restart it, never edit `devbridge.db` by hand.
