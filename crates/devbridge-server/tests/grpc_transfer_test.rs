...
async fn start_server(queue: Arc<JobQueue>, spool_dir: std::path::PathBuf) -> std::net::SocketAddr {
    ...
    let serial_bridge = Arc::new(devbridge_server::serial_bridge::SerialBridgeManager::new(vec![]));
    let dispatch = DispatchService::new(Arc::clone(&queue), spool_dir, connected_clients, 3, serial_bridge);
